...
#include "Render/AdDeferredPBRLightingPass.h"
#include "Render/AdMesh.h"
#include "Render/AdPBRDebugCompositePass.h"
...
protected:
    ...
    void OnInit() override {
        ...
        mLightingPass = std::make_shared<ade::AdDeferredPBRLightingPass>(mLightingRenderPass.get(), mRenderer->GetFramesInFlight());
        mDebugCompositePass = std::make_shared<ade::AdPBRDebugCompositePass>(mPresentRenderPass.get(), mRenderer->GetFramesInFlight());
    ...
    void OnSceneInit(ade::AdScene *scene) override {
        ade::AdEntity *camera = scene->CreateEntity("Deferred PBR Camera");
        ...
        {
            ade::AdEntity *cube = scene->CreateEntity("PBR Cube Low Roughness");
            ...
            transComp.scale = { 0.55f, 0.55f, 0.55f };
            transComp.position = { -1.f, 0.f, 0.f };
            transComp.rotation = { 17.f, 30.f, 0.f };
        }
        {
            ade::AdEntity *cube = scene->CreateEntity("PBR Cube High Roughness");
            auto &materialComp = cube->AddComponent<ade::AdBaseMaterialComponent>();
        ...
        {
            ade::AdEntity *cube = scene->CreateEntity("PBR Cube Metallic");
            auto &materialComp = cube->AddComponent<ade::AdBaseMaterialComponent>();
...
private:
    ...
    void BuildGraphPasses() {
        ...
        mGraph->AddPass("RG.DeferredPBRLighting", { mGBufferBaseColor, mGBufferNormal, mGBufferMaterial, mGBufferWorldPosition }, { mLightingColor },
            [this](ade::AdRenderGraphContext &context) {
        ...
        mGraph->AddPass("RG.PBRDebugComposite", { mLightingColor, mGBufferBaseColor, mGBufferNormal, mGBufferMaterial }, {},
            [this](ade::AdRenderGraphContext &context) {
    ...
    void SetupDebugInput() {
        ...
        mObserver->OnEvent<ade::AdKeyPressEvent>([this](const ade::AdKeyPressEvent &event){
            ...
            switch (event.mKeyCode) {
                case ade::KEY_1:
                    mDebugViewMode = ade::AdPBRDebugViewMode::Lit;
                    LOG_I("PBR debug view: Lit");
                    break;
                case ade::KEY_2:
                    mDebugViewMode = ade::AdPBRDebugViewMode::BaseColor;
                    LOG_I("PBR debug view: BaseColor");
                    break;
                case ade::KEY_3:
                    mDebugViewMode = ade::AdPBRDebugViewMode::Normal;
                    LOG_I("PBR debug view: Normal");
                    break;
                case ade::KEY_4:
                    mDebugViewMode = ade::AdPBRDebugViewMode::Roughness;
                    LOG_I("PBR debug view: Roughness");
                    break;
                case ade::KEY_5:
                    mDebugViewMode = ade::AdPBRDebugViewMode::Metallic;
                    LOG_I("PBR debug view: Metallic");
                    break;
                default:
                    break;
            }
    ...
    std::shared_ptr<ade::AdDeferredPBRLightingPass> mLightingPass;
    std::shared_ptr<ade::AdPBRDebugCompositePass> mDebugCompositePass;
    ade::AdPBRDebugViewMode mDebugViewMode = ade::AdPBRDebugViewMode::Lit;
    ade::AdDeferredPBRLightSettings mLightSettings{};
