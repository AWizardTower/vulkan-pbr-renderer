...
protected:
    ...
    void OnInit() override {
        ...
        mLightingPass = std::make_shared<ade::AdRealIBLDeferredLightingPass>(mLightingRenderPass.get(), mRenderer->GetFramesInFlight());
    ...
    void OnRender() override {
        ...
        if((mNeedsIBLPrecompute || mContinuousIBLPrecompute) && mIBLPrecomputePass && mEnvironmentMap && mIBLResources){
            mIBLPrecomputePass->Precompute(cmdBuffer, nullptr, mEnvironmentMap.get(), mIBLResources.get());
            mNeedsIBLPrecompute = false;
        }
...
private:
    ...
    void UpdateLightMatrices() {
        ...
        mDebugSettings.debugParams = glm::vec4(static_cast<float>(mDebugViewMode), 1.f, 2.2f, mPrefilterDebugMip);
    }
    ...
    void BuildGraphPasses() {
        ...
        mGraph->AddPass("RG.RealIBLDeferredLighting", { mGBufferBaseColor, mGBufferNormal, mGBufferMaterial, mGBufferWorldPosition, mShadowDepth }, { mLightingColor },
            [this](ade::AdRenderGraphContext &context) {
        ...
        mGraph->AddPass("RG.RealIBLDebugComposite", { mLightingColor, mGBufferBaseColor, mGBufferNormal, mGBufferMaterial, mGBufferWorldPosition, mShadowDepth }, {},
            [this](ade::AdRenderGraphContext &context) {
    ...
    void SetupDebugInput() {
        ...
        mObserver->OnEvent<ade::AdKeyPressEvent>([this](const ade::AdKeyPressEvent &event){
            ...
            switch (event.mKeyCode) {
                case ade::KEY_1:
                    mDebugViewMode = ade::AdRealIBLDebugViewMode::Lit;
                    LOG_I("Real IBL debug view: Lit + Sky");
                    break;
                case ade::KEY_2:
                    mDebugViewMode = ade::AdRealIBLDebugViewMode::EnvironmentCube;
                    LOG_I("Real IBL debug view: EnvironmentCube");
                    break;
                case ade::KEY_3:
                    mDebugViewMode = ade::AdRealIBLDebugViewMode::IrradianceCube;
                    LOG_I("Real IBL debug view: IrradianceCube");
                    break;
                case ade::KEY_4:
                    mDebugViewMode = ade::AdRealIBLDebugViewMode::PrefilteredCube;
                    LOG_I("Real IBL debug view: PrefilteredCube mip {0}", mPrefilterDebugMip);
                    break;
                case ade::KEY_5:
                    mDebugViewMode = ade::AdRealIBLDebugViewMode::BRDFLUT;
                    LOG_I("Real IBL debug view: BRDF LUT");
                    break;
                case ade::KEY_6:
                    mDebugViewMode = ade::AdRealIBLDebugViewMode::ShadowFactor;
                    LOG_I("Real IBL debug view: ShadowFactor");
                    break;
                case ade::KEY_7:
                    mDebugViewMode = ade::AdRealIBLDebugViewMode::BaseColor;
                    LOG_I("Real IBL debug view: BaseColor");
                    break;
                case ade::KEY_8:
                    mDebugViewMode = ade::AdRealIBLDebugViewMode::Normal;
                    LOG_I("Real IBL debug view: Normal");
                    break;
                case ade::KEY_9:
                    mDebugViewMode = ade::AdRealIBLDebugViewMode::Roughness;
                    LOG_I("Real IBL debug view: Roughness");
                    break;
                case ade::KEY_0:
                    mDebugViewMode = ade::AdRealIBLDebugViewMode::Metallic;
                    LOG_I("Real IBL debug view: Metallic");
                    break;
                case ade::KEY_LEFT_BRACKET:
                    mPrefilterDebugMip = glm::max(0.f, mPrefilterDebugMip - 1.f);
                    LOG_I("Real IBL prefiltered debug mip: {0}", mPrefilterDebugMip);
                    break;
                case ade::KEY_RIGHT_BRACKET:
                    if(mIBLResources){
                        float maxMip = static_cast<float>(mIBLResources->GetPrefilterMipLevels() - 1);
                        mPrefilterDebugMip = glm::min(maxMip, mPrefilterDebugMip + 1.f);
                    }
                    LOG_I("Real IBL prefiltered debug mip: {0}", mPrefilterDebugMip);
                    break;
                case ade::KEY_R:
                    mContinuousIBLPrecompute = !mContinuousIBLPrecompute;
