...
namespace ade{
    ...
    AdVKDevice::AdVKDevice(AdVKGraphicContext *context, uint32_t graphicQueueCount, uint32_t presentQueueCount, const AdVkSettings &settings) : mContext(context), mSettings(settings) {
        ...
        vkGetPhysicalDeviceProperties(context->GetPhyDevice(), &mPhyDeviceProperties);
        ...
        vkGetPhysicalDeviceQueueFamilyProperties(context->GetPhyDevice(), &queueFamilyCount, nullptr);
        std::vector<VkQueueFamilyProperties> queueFamilyProperties(queueFamilyCount);
        vkGetPhysicalDeviceQueueFamilyProperties(context->GetPhyDevice(), &queueFamilyCount, queueFamilyProperties.data());
        if(graphicQueueFamilyInfo.queueFamilyIndex >= 0 && graphicQueueFamilyInfo.queueFamilyIndex < queueFamilyCount){
            mGraphicsQueueFamilyProperties = queueFamilyProperties[graphicQueueFamilyInfo.queueFamilyIndex];
            bTimestampQuerySupported = mGraphicsQueueFamilyProperties.timestampValidBits > 0
                    && mPhyDeviceProperties.limits.timestampPeriod > 0.0f;
        }
        LOG_D("GPU timestamp query: {0}, timestamp period: {1} ns", bTimestampQuerySupported, mPhyDeviceProperties.limits.timestampPeriod);
