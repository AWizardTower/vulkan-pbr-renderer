...
namespace ade{
    AdDeferredPBRLightingPass::AdDeferredPBRLightingPass(AdVKRenderPass *renderPass, uint32_t framesInFlight)
        ...
        std::vector<VkDescriptorPoolSize> poolSizes = {
            {
                ...
                .descriptorCount = mFramesInFlight * 4
            }
        };
        ...
        VkPushConstantRange lightPC = {
            .stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT,
            .offset = 0,
            .size = sizeof(AdDeferredPBRLightSettings)
        };
        ...
        mPipelineLayout = std::make_shared<AdVKPipelineLayout>(device,
                                                               AD_RES_SHADER_DIR"06_deferred_pbr.vert",
                                                               AD_RES_SHADER_DIR"06_deferred_pbr.frag",
                                                               shaderLayout);
        ...
        mSourceImages.resize(mFramesInFlight, { VK_NULL_HANDLE, VK_NULL_HANDLE, VK_NULL_HANDLE, VK_NULL_HANDLE });
        mSourceViews.resize(mFramesInFlight);
    }

    AdDeferredPBRLightingPass::~AdDeferredPBRLightingPass() {
        mSourceViews.clear();
        mPipeline.reset();
    ...
    void AdDeferredPBRLightingPass::Render(VkCommandBuffer cmdBuffer,
        ...
        if(!renderTarget || !baseColor || !normal || !material || !worldPosition || mFramesInFlight == 0 || mDescriptorSets.empty()){
            return;
        }

        frameSlot %= mFramesInFlight;
        UpdateDescriptor(frameSlot, baseColor, normal, material, worldPosition);
    ...
    void AdDeferredPBRLightingPass::InvalidateSourceViews() {
        std::fill(mSourceImages.begin(), mSourceImages.end(), std::array<VkImage, 4>{ VK_NULL_HANDLE, VK_NULL_HANDLE, VK_NULL_HANDLE, VK_NULL_HANDLE });
        mSourceViews.clear();
        mSourceViews.resize(mFramesInFlight);
    }

    void AdDeferredPBRLightingPass::UpdateDescriptor(uint32_t frameSlot,
                                                     AdVKImage *baseColor,
                                                     AdVKImage *normal,
                                                     AdVKImage *material,
                                                     AdVKImage *worldPosition) {
        if(!baseColor || !normal || !material || !worldPosition || frameSlot >= mSourceImages.size()){
            return;
        }

        std::array<AdVKImage*, 4> sourceImages = { baseColor, normal, material, worldPosition };
        std::array<VkImage, 4> sourceHandles = {
            baseColor->GetHandle(),
            normal->GetHandle(),
            material->GetHandle(),
