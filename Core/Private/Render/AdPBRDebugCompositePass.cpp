...
namespace ade{
    AdPBRDebugCompositePass::AdPBRDebugCompositePass(AdVKRenderPass *renderPass, uint32_t framesInFlight)
        ...
        mPipelineLayout = std::make_shared<AdVKPipelineLayout>(device,
                                                               AD_RES_SHADER_DIR"06_pbr_debug_composite.vert",
                                                               AD_RES_SHADER_DIR"06_pbr_debug_composite.frag",
                                                               shaderLayout);
    ...
    AdPBRDebugCompositePass::~AdPBRDebugCompositePass() {
        mSourceViews.clear();
        mPipeline.reset();
    ...
    void AdPBRDebugCompositePass::Render(VkCommandBuffer cmdBuffer,
        ...
        if(!renderTarget || !lightingColor || !baseColor || !normal || !material || mFramesInFlight == 0 || mDescriptorSets.empty()){
            return;
        }

        frameSlot %= mFramesInFlight;
        UpdateDescriptor(frameSlot, lightingColor, baseColor, normal, material);
    ...
    void AdPBRDebugCompositePass::InvalidateSourceViews() {
    ...
    void AdPBRDebugCompositePass::UpdateDescriptor(uint32_t frameSlot,
                                                   AdVKImage *lightingColor,
                                                   AdVKImage *baseColor,
                                                   AdVKImage *normal,
                                                   AdVKImage *material) {
        if(!lightingColor || !baseColor || !normal || !material || frameSlot >= mSourceImages.size()){
            return;
        }

        std::array<AdVKImage*, 4> sourceImages = { lightingColor, baseColor, normal, material };
