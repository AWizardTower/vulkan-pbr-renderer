...
namespace ade{
    AdShadowDebugCompositePass::AdShadowDebugCompositePass(AdVKRenderPass *renderPass, uint32_t framesInFlight)
        ...
        VkPushConstantRange debugPC = {
            .stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT,
            .offset = 0,
            .size = sizeof(AdShadowDebugSettings)
        };
        ...
        mPipelineLayout = std::make_shared<AdVKPipelineLayout>(device,
                                                               AD_RES_SHADER_DIR"07_shadow_debug_composite.vert",
                                                               AD_RES_SHADER_DIR"07_shadow_debug_composite.frag",
                                                               shaderLayout);
    ...
    AdShadowDebugCompositePass::~AdShadowDebugCompositePass() {
        mSourceViews.clear();
        mPipeline.reset();
    ...
    void AdShadowDebugCompositePass::Render(VkCommandBuffer cmdBuffer,
        ...
        if(!renderTarget || !lightingColor || !baseColor || !normal || !material || !worldPosition || !shadowDepth || mFramesInFlight == 0 || mDescriptorSets.empty()){
            return;
        }

        frameSlot %= mFramesInFlight;
        UpdateDescriptor(frameSlot, lightingColor, baseColor, normal, material, worldPosition, shadowDepth);
    ...
    void AdShadowDebugCompositePass::InvalidateSourceViews() {
    ...
    void AdShadowDebugCompositePass::UpdateDescriptor(uint32_t frameSlot,
                                                      AdVKImage *lightingColor,
                                                      AdVKImage *baseColor,
                                                      AdVKImage *normal,
                                                      AdVKImage *material,
                                                      AdVKImage *worldPosition,
                                                      AdVKImage *shadowDepth) {
        if(!lightingColor || !baseColor || !normal || !material || !worldPosition || !shadowDepth || frameSlot >= mSourceImages.size()){
            return;
        }
