#ifndef AD_REAL_IBL_DEBUG_COMPOSITE_PASS_H
#define AD_REAL_IBL_DEBUG_COMPOSITE_PASS_H
...
namespace ade{
    ...
    enum class AdRealIBLDebugViewMode : uint32_t{
        Lit = 1,
        EnvironmentCube = 2,
        IrradianceCube = 3,
        PrefilteredCube = 4,
    ...
    struct AdRealIBLDebugSettings{
        glm::mat4 inverseViewProj{ 1.f };
        glm::mat4 lightViewProj{ 1.f };
        glm::vec4 cameraPosition{ 0.f, 0.f, 2.3f, 0.f };
        glm::vec4 debugParams{ static_cast<float>(AdRealIBLDebugViewMode::Lit), 1.f, 2.2f, 0.f };
        glm::vec4 shadowParams{ 1.f / 2048.f, 0.0015f, 0.f, 0.f };
        glm::vec4 iblParams{ 0.55f, 1.0f, 0.f, 0.f };
    };

    class AdRealIBLDebugCompositePass{
    public:
        AdRealIBLDebugCompositePass(AdVKRenderPass *renderPass, uint32_t framesInFlight);
        ~AdRealIBLDebugCompositePass();
    ...
    private:
        ...
        std::vector<std::array<VkImage, 10>> mSourceImages;
