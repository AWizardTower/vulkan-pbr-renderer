...
layout(set = 0, binding = 6) uniform samplerCube environmentCube;
layout(set = 0, binding = 7) uniform samplerCube irradianceCube;
layout(set = 0, binding = 8) uniform samplerCube prefilteredCube;
layout(set = 0, binding = 9) uniform sampler2D brdfLUT;

layout(set = 0, binding = 10) uniform RealIBLDebugSettings{
...
void main() {
    ...
    float prefilterMip = clamp(Settings.debugParams.w, 0.0, max(float(textureQueryLevels(prefilteredCube) - 1), 0.0));
    vec3 skyColor = textureLod(environmentCube, viewRay, 0.0).rgb;
    ...
    if(debugViewMode == 3u) {
        outColor = vec4(ToneMap(texture(irradianceCube, viewRay).rgb), 1.0);
        return;
    }
    if(debugViewMode == 4u) {
        outColor = vec4(ToneMap(textureLod(prefilteredCube, viewRay, prefilterMip).rgb), 1.0);
        return;
    }
    if(debugViewMode == 5u) {
        outColor = vec4(texture(brdfLUT, fragUV).rg, 0.0, 1.0);
        return;
    }

    if(!hasGeometry) {
        outColor = debugViewMode == 1u ? vec4(ToneMap(skyColor), 1.0) : vec4(0.0, 0.0, 0.0, 1.0);
        return;
    }
    ...
    } else if(debugViewMode == 7u) {
        outColor = vec4(baseColor, 1.0);
    } else if(debugViewMode == 8u) {
        outColor = vec4(N * 0.5 + 0.5, 1.0);
    } else if(debugViewMode == 9u) {
        outColor = vec4(RoughnessHeatmap(roughness), 1.0);
    } else if(debugViewMode == 10u) {
