...
layout(push_constant) uniform IBLLightPushConstants{
    mat4 lightViewProj;
    vec4 cameraPosition;
    vec4 lightDirection;
...
float SampleShadowFactor(vec3 worldPosition) {
    ...
    float texelSize = PC.iblParams.x;
    float depthBias = PC.iblParams.y;
...
void main() {
    ...
    vec3 envDiffuse = SampleEnvironment(N, maxEnvMip) * baseColor * (1.0 - metallic) * PC.iblParams.z;
    vec3 R = reflect(-V, N);
    vec3 envSpecular = SampleEnvironment(R, roughness * maxEnvMip) * FresnelSchlick(NoV, F0) * PC.iblParams.w;

    outColor = vec4(directLighting + envDiffuse + envSpecular, 1.0);
}
