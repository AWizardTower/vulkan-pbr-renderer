...
layout(set = 0, binding = 5) uniform samplerCube irradianceCube;
layout(set = 0, binding = 6) uniform samplerCube prefilteredCube;
layout(set = 0, binding = 7) uniform sampler2D brdfLUT;

layout(push_constant) uniform RealIBLLightPushConstants{
    mat4 lightViewProj;
    vec4 cameraPosition;
    vec4 lightDirection;
